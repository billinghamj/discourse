...
#   exceptions were caught by the `handle_exceptions` block. One or more
#   exception classes can be provided to specifically handle those exceptions.
#
  ...
  AVAILABLE_ACTIONS = {
    ...
    on_exceptions: {
      condition: ->(*exceptions) do
        next unless failure_for?("result.rescue.default")
        next true if exceptions.empty?
        exceptions.any? { result["result.rescue.default"].exception.is_a?(_1) }
      end,
      key: %w[result rescue],
      name: "default",
      property: :exception,
